...
Version in `analysis/bot/config.py` `VERSION` constant (currently 11.3.1). Dashboard on `:8097`.
...
- **S9 early exit**: If unrealized < -1000 bps after 8h, close immediately. Winners revert fast (+1006 bps at 4h), losers keep going (-546 bps). At -1000 bps threshold, 95% of trades below are losers (23.9:1 ratio). Backtest: +18% total P&L ($75k→$88k). **Not generalizable**: S5/S8 early exits tested and both destroy value in compounding (-31% to -35%) because those signals need more time to revert.

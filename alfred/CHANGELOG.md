...
- **R&D**: audit du modèle de coûts sur les fills réels avant décision, motivé par le fait qu'une erreur de coût par trade biaise systématiquement pour ou contre le signal le plus actif. La méthode historique s'est révélée sans puissance sur un échantillon récent — sa référence est postérieure à la décision, elle mesure la dérive de prix plus que l'exécution. Mesure refaite contre le prix vu au moment de l'ordre : le modèle du backtest n'est pas réfuté, mais il sous-facture probablement, donc il favorise les configurations qui tradent le plus. Traité en sensibilité imposée plutôt qu'en re-calage, un re-calage sur si peu de trades allant dans le sens de la conclusion attendue.
- **R&D**: le fait structurel « seuls les signaux de divergence les plus FAIBLES sont rentables » survit à la correction, sur les trois fenêtres — alors que le moteur trie par force décroissante et priorise donc les pires candidats. Conformément à la grille, cet argument ne servait qu'à départager un verdict serré ; il ne l'était pas. Devient la piste prioritaire, à instruire comme étude neuve avec sa propre grille.

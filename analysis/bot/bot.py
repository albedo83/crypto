            ...
            try:
                ...
                if now - self._last_scan >= SCAN_INTERVAL or _post_4h_close:
                    ...
                    for sym in ALL_SYMBOLS:
                        ...
                        # v12.16.6: 0.2s → 0.5s. v12.17.2: configurable via env
                        # CANDLE_FETCH_SLEEP. Paper / apprenti set this to 1.0s+
                        # to reduce shared-IP burst at 4h close (4 bots × 35
                        # tokens = 140 fetches in ~20s if all run at 0.5s).
                        from .config import CANDLE_FETCH_SLEEP
                        await asyncio.sleep(CANDLE_FETCH_SLEEP)

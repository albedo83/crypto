...
**Dead-in-water exit (v12.5.37)** (`backtest_s8_dead_in_water.py`, walk-forward 28m/12m/6m positifs + 3m null intersection) : sur les positions S8 **LONG**, à T+8h après l'entrée, si `pos.mfe_bps ≤ 50` (la position n'a jamais respiré au-dessus de +0.5% MFE), exit immédiat. Mécanique : un vrai bottom de capitulation génère un MFE rapide via chasse aux liquidités et rachats de shorts. Pas de rebond à T+8h = thèse invalidée, la pression vendeuse absorbe chaque tentative. Discovery via mid-trade profiling EDA (`mid_trade_profiling_eda.md`, null-shuffle z=−6.41, savings +192 bps/cut in-sample). Walk-forward : 28m +207 872pp / 12m +1 723pp / 6m +138pp / 3m 0pp (la règle n'a pas trigger sur 3m, 5 S8 LONG mais aucun ≤ 50 bps à T+8h). DD jamais dégradée (avg 0 sur 28m/12m/3m, **+8.39pp amélioration** sur 6m). 11 cuts 28m : 9 genuine (sauvent −400 à −690 bps de pertes) + 2 stragglers (GMX +57, CRV +607) = net +3 209 bps. Block placé entre `s10_trailing` et le block `s8_inlife` (mécaniquement disjoint : MFE ≤ 50 vs MFE ≥ 300/1500, queues opposées de la distribution). `pos.mfe_bps` est monotone non-décroissant donc le check est naturellement idempotent : une fois MFE > 50, la règle ne fire plus jamais. Kill-switch : `S8_DEAD_MFE_MAX_BPS = -99999` dans `config.py`. New exit reason : `s8_dead_in_water`.

S5 a été testé sur les 3 familles (A.1 global, A.2 régime, B percentile, C ML) — **aucune ne produit de candidate robuste** sur S5. Les winners S5 ont une distribution bimodale (certains gigantesques, certains modestes) ; cutter à seuil fixe ampute le tail. Mid-trade EDA + walk-forward 4/4 sur S5 LONG (T+8h) ont confirmé ce verdict 2026-05-15 : signature in-sample propre (z=−6.41) mais 1/4 strict OOS. Pour les S5-like (INJ, BLUR), l'override manuel via 🎯 reste la stratégie de référence.
...
| **Adaptive modulator** (v11.10.0 + v12.2.0) | S1×(1+0.5·btc_z), S8×(1−0.5·btc_z), S9×(1−0.5·btc_z), S5 SHORT×(1−0.5·btc_z) — non actif sur S5 LONG ni S10 | btc_z = z-score rolling 6m du return BTC 30j. Reflète la régime-dependance de chaque signal. v12.2.0 ajoute S5 SHORT (override directionnel via `ADAPTIVE_ALPHA_DIR`) car shorter en bull vs outperformers = perte structurelle (catastrophe SEI 2026-05-08). Walk-forward 4/4 strict (28m +10100pp, ΔDD +1.5pp). Multiplier clip [0.3, 2.5]. |
...
| **S8 dead-in-water exit** (v12.5.37) | À T+8h, si S8 LONG a `mfe_bps ≤ 50` (jamais respiré +0.5%) → cut immédiat. | Nettoie les S8 amorphes sans pouls. Pair opposé du trail (MFE ≤ 50 vs ≥ 300/1500). Validé walk-forward 28m/12m/6m positifs + 3m null + DD jamais dégradée. |
...
- Trailing stop generalise S5/S8/S9 (miroir per-strat du trailing S10) — `backtest_trailing_sweep.py` → rejete : 45 combos (3 strats × 5 triggers {300,400,500,600,800} × 3 offsets {100,150,200}), 0 passe le 4/4. S5 catastrophique (avg ΔPnL -1800 a -2200pp), S8 mauvais (-400 a -1400), S9 le moins pire (-50 a -700) avec quelques 1/4 isolees. Le pattern observe en LIVE 60-trades (trailing semble sauver $50-300) ne tient pas sur 1100+ trades : le bot tire son edge des timeouts patients qui passent par des dips (MFE +400 → dip → MFE +800 → timeout +500). Trailing coupe au premier dip et rate la vraie sortie. Confirme que l'asymetrie loss/win est structurelle.
- Dynamic exit on BTC adverse move during hold — `backtest_btc_corr_exit.py` (first-order post-hoc beta=1) puis `backtest_btc_corr_exit_v2.py` (in-engine). LIVE+PAPER post-hoc montre une vraie correlation : LONG perdent quand BTC dump pendant le hold, SHORT perdent quand BTC rally. Hypothese : couper la position quand BTC bouge X% contre la direction dans les Y premieres heures. First-order post-hoc beta=1 surface 5 candidats 4/4 (top: thr=300 lb=12h LONG +73pp). In-engine v2 (36 combos thr {300,500,800} × lookback {full,24h,12h,8h} × scope {LONG, SHORT, LONG+SHORT}) : **0 robuste 4/4 avec DD intact**. Les beta-reels (1.5-2.5 sur alts) + l'effet de chemin (slot libere → autre trade) detruisent le benefice apparent. La correlation BTC est deja reflee par les price-stops existants. Apprentissage meta : un first-order post-hoc beta=1 produit des faux positifs systematiques, le walk-forward in-engine est essentiel.

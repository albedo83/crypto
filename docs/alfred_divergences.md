...
| 10 | Calcul du btc_z | `features.compute_btc_z` à chaque scan (fenêtre glissante sur le deque) | map vectorisée précalculée dans `run_window` (+ variantes v12.18.x) | les deux implémentations subsistent — TODO : test de parité dédié avant phase 6 |

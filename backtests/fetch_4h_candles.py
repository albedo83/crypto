"""Refresh 4h candle history (`*_4h_3y.json`) for all backtest tokens.

Idempotent: reads existing JSON, finds last timestamp, requests new candles
from Hyperliquid's `candleSnapshot` endpoint, dedupes, writes back. Safe to
run repeatedly; cheap when data is already fresh.

Usage:
    python3 -m backtests.fetch_4h_candles
    python3 -m backtests.fetch_4h_candles --symbols BTC,ETH
    python3 -m backtests.fetch_4h_candles --full  # ignore cache, refetch 3y
"""
...
def fetch_window(coin: str, start_ms: int, end_ms: int) -> list:
    """Fetch one chunk of 4h candles (HL caps at 5000 per request)."""
    payload = json.dumps({
        "type": "candleSnapshot",
...
def update_token(coin: str, full: bool = False) -> dict:
    ...
    fallback = os.path.join(DATA_DIR, f"{coin}_4h.json")
    existing: list = []
    if not full and os.path.exists(path):
        with open(path) as f:
            existing = json.load(f)
    elif not full and os.path.exists(fallback):
...
def main():
    ...
    ap.add_argument("--full", action="store_true", help="Ignore cache, refetch 3y")
    args = ap.parse_args()

    symbols = (args.symbols.split(",") if args.symbols
               else TOKENS + REF_TOKENS)
    print(f"Refreshing 4h candles for {len(symbols)} tokens "

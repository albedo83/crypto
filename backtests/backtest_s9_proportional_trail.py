"""Walk-forward strict — S9 proportional trail.

stop = arm_bps + (mfe - arm_bps) × lock_ratio

Sweep (arm_bps, lock_ratio). PASS = strict 4/4 ΔPnL > 0 AND ΔDD >= -2pp.

Usage: python3 -m backtests.backtest_s9_proportional_trail
"""
...
# Grid — proportional trail
ARMS = [300, 500, 800, 1000, 1200]
LOCKS = [0.50, 0.60, 0.667, 0.75, 0.85]
...
def main():
    ...
    print("=" * 84)
    print("BASELINE (no proportional trail)")
    print("=" * 84)
    baseline = {}
    ...
    results = []
    total = len(ARMS) * len(LOCKS)
    idx = 0
    for arm in ARMS:
        for lock in LOCKS:
            idx += 1
            cfg = {"strategy": "S9", "arm_bps": arm, "lock_ratio": lock}
            per_split = {}
            roi_pass = dd_pass = 0
            sum_dpnl = sum_ddd = 0.0
            print(f"\n[{idx}/{total}] S9 prop_trail arm={arm} lock={lock:.3f}", flush=True)
            for name, sd, ed in SPLITS:
    ...
    print(f"{'Arm':>5} {'Lock':>6} {'ROI 4/4':>9} {'DD 4/4':>9} {'sum ΔPnL':>12} {'avg ΔDD':>10}  Verdict")
    for r in results:
        strict = (r["roi_pass"] == 4 and r["dd_pass"] == 4)
        verdict = "STRICT PASS 4/4" if strict else f"FAIL ({r['roi_pass']}/4 ROI, {r['dd_pass']}/4 DD)"
        print(f"{r['arm']:>5} {r['lock']:>6.3f} {r['roi_pass']:>9}/4 {r['dd_pass']:>9}/4 {r['sum_dpnl']:>+11,.0f}$ {r['avg_ddd']:>+9.2f}pp  {verdict}")
    ...
    print("=" * 84)
    for r in results[:3]:
        print(f"\n  S9 prop_trail arm={r['arm']} lock={r['lock']:.3f}  sum_ΔPnL=${r['sum_dpnl']:+,.0f}  avg_ΔDD={r['avg_ddd']:+.2f}pp")

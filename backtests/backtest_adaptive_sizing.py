"""Adaptive per-strategy sizing based on rolling WR.

Continuous multiplier instead of binary kill-switch. When a strategy runs cold,
size shrinks but doesn't zero; when it runs hot, size expands. This avoids the
"miss the rebound" failure mode of the pure kill-switch.

Formula: size_mult = clip(WR / center * scale, min_mult, max_mult)

Variants tested:
- (0.5, 1.5) with center=0.5 (linear WR scaling)
- (0.3, 1.5)
- (0.5, 1.2) — conservative upside
- (0.3, 1.0) — penalty only, no bonus
- EMA-based WR with (0.5, 1.5)
- Sharpe-based scaling (mean_bps / std over rolling N)

Only real trades count for the rolling stats. No shadow trades (the strategy
is always active — we just dampen/amplify its size).
"""
...
def run_window(features, data, sector_features,
               ...
               window_n=0, min_mult=1.0, max_mult=1.0,
               mode="wr", center=0.5):
    """Run backtest with adaptive per-strategy sizing.

    Args:
        window_n: rolling window size (0 = disabled, flat 1.0 mult).
        min_mult, max_mult: clip range for the multiplier.
        mode: 'wr' (rolling WR / center), 'sharpe' (rolling mean_bps/std).
        center: WR reference (typically 0.5 = 50%).
    """
    ...
    strat_hist = {s: deque(maxlen=max(window_n, 1)) for s in STRATEGIES}
    strat_mult_log = {s: [] for s in STRATEGIES}

    def strat_mult(strat):
        if window_n == 0:
            return 1.0
        h = strat_hist[strat]
        if len(h) < window_n:
        ...
        if mode == "wr":
            wins = sum(1 for x in h if x > 0)
            wr = wins / len(h)
            return max(min_mult, min(max_mult, wr / center))
        elif mode == "sharpe":
            arr = np.array(h, dtype=float)
            if arr.std() == 0:
                return 1.0
            sharpe = arr.mean() / arr.std()
            # sharpe ~ 0 => 1.0, sharpe > 0.3 => boost, < -0.3 => cut
            return max(min_mult, min(max_mult, 1.0 + sharpe))
        return 1.0
    ...
    for ts in sorted_ts:
        ...
        for coin in list(positions.keys()):
            ...
            if exit_reason:
                ...
                if mode == "wr":
                    strat_hist[pos["strat"]].append(1 if pnl > 0 else 0)
                else:  # sharpe uses net bps
                    strat_hist[pos["strat"]].append(net)
                del positions[coin]
        ...
        for cand in filtered:
            ...
            # Adaptive sizing: base × per-strategy multiplier
            base_size = strat_size(cand["strat"], capital)
            mult = strat_mult(cand["strat"])
            size = round(max(10, base_size * mult), 2)
            strat_mult_log[cand["strat"]].append(mult)
    ...
    mean_mult = {s: (sum(v) / len(v) if v else 1.0) for s, v in strat_mult_log.items()}
    min_mult_obs = {s: (min(v) if v else 1.0) for s, v in strat_mult_log.items()}
    max_mult_obs = {s: (max(v) if v else 1.0) for s, v in strat_mult_log.items()}
    ...
    return {
        ...
        "mean_mult": mean_mult,
        "min_mult": min_mult_obs,
        "max_mult": max_mult_obs,
    }
...
def main():
    ...
    # (label, N, min_mult, max_mult, mode, center)
    configs = [
        ("WR N=20 [0.5, 1.5]", 20, 0.5, 1.5, "wr", 0.5),
        ("WR N=20 [0.3, 1.5]", 20, 0.3, 1.5, "wr", 0.5),
        ("WR N=20 [0.5, 1.2]", 20, 0.5, 1.2, "wr", 0.5),
        ("WR N=20 [0.3, 1.0]", 20, 0.3, 1.0, "wr", 0.5),  # penalty-only
        ("WR N=30 [0.5, 1.5]", 30, 0.5, 1.5, "wr", 0.5),
        ("WR N=15 [0.5, 1.5]", 15, 0.5, 1.5, "wr", 0.5),
        ("WR N=20 [0.4, 1.3]", 20, 0.4, 1.3, "wr", 0.5),
        ("SHR N=20 [0.5, 1.5]", 20, 0.5, 1.5, "sharpe", 0.0),
        ("SHR N=30 [0.5, 1.5]", 30, 0.5, 1.5, "sharpe", 0.0),
    ]

    print(f"\n{'='*110}")
    print(f"Adaptive sizing backtest — v{VERSION} — data thru {end_dt.date()}")
    print(f"{'='*110}\n")
    ...
    for label, start_dt in windows:
        ...
        for cfg_label, N, mn, mx, mode, ctr in configs:
            r = run_window(features, data, sector_features, start_ts, latest_ts,
                           window_n=N, min_mult=mn, max_mult=mx, mode=mode, center=ctr)
            dp = r["pnl"] - base_pnl
            dd = r["max_dd_pct"] - base_dd
            # Show per-strat mean multiplier (tells us if a strat was scaled down on average)
            mult_hint = " ".join(f"{s}:{r['mean_mult'][s]:.2f}" for s in ["S5", "S9", "S10"])
            print(f"  {cfg_label:22}: {fmt_result(r)} | Δ${dp:+.0f} ΔDD {dd:+.1f}pp | avg mult {mult_hint}")
